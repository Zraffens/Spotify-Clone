      ...
      <tbody>
        ...
        >
          ...
          <td style="width: 22%;">
            ...
            <router-link to="/" class="artist">{{ song.artist }}</router-link>
          </td>
  ...
  async created() {
    ...
    if (this.$route.path === `/playlists/${this.id}`) {
      ...
      await axios.get(url).then((res) => {
        ...
        res.data.artists.forEach(async (id) => {
          await axios
        ...
        songs.forEach(async (id) => {
          await axios
            ...
            .then(async (res2) => {
              let song = { ...res2.data };
              this.totalLength = this.totalLength + song.length;
